/**********************************************************************
  ...
  rst_output.c - functions for printing Elements parsed by 
                      rst_peg.
...
static void print_html_element(GString *out, element *elt, bool obfuscate) {
    ...
    case H1TITLE:
        pad(out, 2);
        g_string_append_printf(out, "<h1 class=\"title\">");
        print_html_element_list(out, elt->children, obfuscate);
        g_string_append_printf(out, "</h1>");
        pad(out, 2);
        padded = 0;
        break;
    ...
    case CODEBLOCK:
        pad(out, 2);
        g_string_append_printf(out, "%s", "<pre class=\"");
        g_string_append_printf(out, "%s", elt->contents.code->lang);
        g_string_append_printf(out, "%s", "\"><code>");
        print_html_string(out, elt->contents.code->str, obfuscate);
