/**********************************************************************
  ...
  rst_lib.c - reStructuredText in C using a PEG grammar.
...
static void print_tree(element * elt, int indent) {
    ...
    while (elt != NULL) {
        ...
        switch (elt->key) {
            ...
            case H1TITLE:            key = "H1TITLE"; break;
            ...
            case TABLE:              key = "TABLE"; break;
            case TABLESEPARATOR:     key = "TABLESEPARATOR"; break;
            case TABLECELL:          key = "TABLECELL"; break;
            case CELLSPAN:           key = "CELLSPAN"; break;
            case TABLEROW:           key = "TABLEROW"; break;
            case TABLEBODY:          key = "TABLEBODY"; break;
            case TABLEHEAD:          key = "TABLEHEAD"; break;
            ...
            case DEFINITIONLIST:     key = "DEFINITIONLIST"; break;
            case DEFTITLE:           key = "DEFTITLE"; break;
            case DEFDATA:            key = "DEFDATA"; break;
            default:                 key = "?";
        }
        if ( elt->key == STR ) {
 ...
 * the result of parsing them as reStructuredText text, and recursing into the children
...
static element * process_raw_blocks(element *input, int extensions, element *references, element *notes) {
    ...
    while (current != NULL) {
        if (current->key == RAW) {
            ...
            current->key = LIST;
            current->children = parse_rst(contents, extensions, references, notes);
            last_child = current->children;
            while ((contents = strtok(NULL, "\001"))) {
                while (last_child->next != NULL)
                    last_child = last_child->next;
                last_child->next = parse_rst(contents, extensions, references, notes);
            }
...
/* rst_to_gstring - convert reStructuredText text to the output format specified.
...
GString * rst_to_g_string(char *text, int extensions, int output_format) {
    ...
    result = parse_rst(formatted_text->str, extensions, references, notes);
...
/* rst_to_string - convert reStructuredText text to the output format specified.
...
char * rst_to_string(char *text, int extensions, int output_format) {
    GString *out;
    char *char_out;
    out = rst_to_g_string(text, extensions, output_format);
    char_out = out->str;
