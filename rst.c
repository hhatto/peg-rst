/**********************************************************************
  ...
  rst.c - reStructuredText in C using a PEG grammar.
  (c) 2017 Hideo Hattori.
...
/**********************************************************************
  ...
  rst_lib.c.  It parses command-line options, reads the text to
...
#define COPYRIGHT "Copyright (c) 2017 Hideo Hattori.  License GPLv2+ or MIT.\n" \
...
int main(int argc, char * argv[]) {
    ...
    g_option_context_set_description (context, "Converts text in specified files (or stdin) from reStructuredText to FORMAT.\n"
    ...
    out = rst_to_string(inputbuf->str, extensions, output_format);
    fprintf(output, "%s\n", out);
    free(out);
