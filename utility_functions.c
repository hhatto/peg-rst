 ...
 * constructors, and macro definitions for leg reStructuredText parser. */
...
element * mk_link(element *label, char *url, char *title) {
    ...
    result->contents.peglink = malloc(sizeof(peglink));
    result->contents.peglink->label = label;
    result->contents.peglink->url = strdup(url);
    result->contents.peglink->title = strdup(title);
    return result;
}

element * mk_codeblock(element *list, char *lang) {
    element *result;
    GString *s = concat_string_list(reverse(list));

    result = mk_element(CODEBLOCK);
    result->contents.code = malloc(sizeof(code));
    result->contents.code->lang = strdup(lang);
    result->contents.code->str = s->str;
    g_string_free(s, false);
    return result;
}

int get_heading_level(char label) {
    int retLevel = -1;
    int level = 0;
    for (level=0; level<6; level++) {
        if (label == heading_keys[level]) {
            retLevel = level;
            break;
        }
    }
    if (retLevel == -1) {
        heading_keys[heading_level] = label;
        retLevel = heading_level;
        heading_level += 1;
    }
    switch (retLevel) {
    case 0:
